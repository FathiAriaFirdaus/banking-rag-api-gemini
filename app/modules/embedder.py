...
class Embedder:
    def __init__(self):
        self.embedding_model = None
        # Gunakan model yang lebih kecil untuk semua environment
        self.model_name = "sentence-transformers/all-mpnet-base-v2"
    
    def get_embedding_model(self):
        """Get embedding model - always load directly from HuggingFace, no pickle"""
        if self.embedding_model is None:
            try:
                logger.info(f"Loading embedding model directly from HuggingFace: {self.model_name}")
                ...
                self.embedding_model = HuggingFaceEmbeddings(
                    ...
                    encode_kwargs={
                        ...
                        'batch_size': 32  # Smaller batch size for Vercel
                    }
                )
                ...
                try:
                    logger.info("Trying fallback model: sentence-transformers/all-mpnet-base-v2")
                    self.embedding_model = HuggingFaceEmbeddings(
